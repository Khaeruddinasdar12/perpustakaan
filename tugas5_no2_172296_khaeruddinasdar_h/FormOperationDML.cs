...
namespace tugas5_no2_172296_khaeruddinasdar_h
{
	...
	//				dr = new MySqlDataReader();
					...
					if (dr.Read() ) {
			            ...
			            alamat.Text= dr[3].ToString();
			            nt.Value = Convert.ToInt32(dr[4]);
			            nm.Value = Convert.ToInt32(dr[5]);
			            nf.Value = Convert.ToInt32(dr[6]);
			            na.Text = dr[7].ToString();
			            nh.Text = dr[8].ToString();
			            keterangan.Text = dr[9].ToString();
